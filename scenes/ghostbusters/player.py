"""Player character: movement, gravity, jump, collision, and animation
state machine -- ported from the original GhostBusters/player.py.
Physics/collision logic is unchanged (see ADR-0002). The only change:
character images now come from the shared, cached AssetManager instead
of a fresh pygame.image.load() per animation frame.

NOTE: WIDTH/HEIGHT here are the game's fixed internal simulation
resolution (640x384), not the shared window's actual size.
GhostBustersScene renders this game onto an internal surface at this
resolution and scales the result up to whatever the shared window
actually is, so all of the original camera/scroll math below works
...
class Player(pygame.sprite.Sprite):
    def __init__(self, x, y, assets):
        ...
        for i in range(1, 3):
            image = assets.get_image(f"ghostbusters/PlayerIdle{i}.png")
            image = pygame.transform.scale(image, (24, 24))
            self.idle_list.append(image)
        for i in range(1, 6):
            image = assets.get_image(f"ghostbusters/PlayerWalk{i}.png")
            right = pygame.transform.scale(image, (24, 24))
            left = pygame.transform.flip(right, True, False)
            self.walk_right.append(right)
            self.walk_left.append(left)
        for i in range(1, 5):
            image = assets.get_image(f"ghostbusters/PlayerAttack{i}.png")
            image = pygame.transform.scale(image, (24, 24))
            self.attack_list.append(image)
        for i in range(1, 11):
            image = assets.get_image(f"ghostbusters/PlayerDead{i}.png")
            image = pygame.transform.scale(image, (24, 24))
            self.death_list.append(image)
        for i in range(1, 3):
            image = assets.get_image(f"ghostbusters/PlayerHit{i}.png")
            image = pygame.transform.scale(image, (24, 24))
